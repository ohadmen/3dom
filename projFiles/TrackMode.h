...
public:
	...
	*/
	virtual void Apply(Trackball * trackball, QVector3D new_point) {}
	/*!
...
public:
	...
	*/
	...
	{
		TrackUtils::DrawSphereIcon(trackball, false);
	}
...
public:
	...
	@param new_point the new mouse pointer coordinate.
	*/
	void Apply(Trackball * trackball, QVector3D new_point)
	{
		QVector3D hitOld = HitSphere(tb, tb->last_point);
		QVector3D hitNew = HitSphere(tb, new_point);
		tb->Hits.push_back(hitNew);
		QVector3D center = tb->center;
		QVector3D axis = (hitNew - center) ^ (hitOld - center);
		axis.normalize();
...
public:
	...
	This constructor can initialize it with a QLine3D.

	The line will be normalized.
	@param ln the line that represent the direction.
	*/
	AxisMode(const QLine3D & ln)
		: axis(ln) {
	}
	/*!
	...
	This manipulator needs to be initialized with a direction.
	This constructor can initialize it with two QVector3D,
	representing a point and a vector.

	@param origin a point on the line.
	@param direction the line direction.
	*/
	AxisMode(const QVector3D & origin, const QVector3D & direction)
		: axis(QLine3D(origin, direction)) {
	}
	/*!
	@brief Return this manipulator's name.
...
public:
	...
	*/
	PlaneMode(float a, float b, float c, float d)
		: plane(QPlane3D(QVector3D(a, b, c),d)) {
	}
	/*!
	@brief The plane constructor.

	This manipulator needs to be initialized with a plane.
	This constructor can initialize it with a QPlane3D.

	@param pl the plane.
	*/
	PlaneMode(QPlane3D & pl)
		: plane(pl) {
	}
	/*!
	@brief Return this manipulator's name.
...
public:
	...
	*/
	CylinderMode(QLine3D & ln, float s = 0.0f)
		: axis(ln), snap(s) {
		assert(snap >= 0.0);
	}
	/*!
	...
	@param s a rotational snap angle (non negative)
	*/
	CylinderMode(const QVector3D & origin, const QVector3D & direction, float s = 0.0f)
		: axis(QLine3D(origin, direction)), snap(s) {
		assert(snap >= 0.0);
	}
	/*!
	@brief Return this manipulator's name.
...
public:
	...
	*/
	PathMode(const std::vector < QVector3D > &pts, bool w = false)
	...
	*/
	PathMode(const QVector3D &start, const QVector3D &end)
	...
	*/
	QVector3D SetStartNear(QVector3D p);
	/*!
...
private:
	...
	*/
	void GetPoints(float state, QVector3D & point, QVector3D & prev_point, QVector3D & next_point);
	/*!
	...
	*/
	float HitPoint(float state, QRay3D ray, QVector3D &hit_point);
	/*!
	...
	*/
	int Verse(QVector3D reference_point, QVector3D current_point, QVector3D prev_point, QVector3D next_point);
...
public:
	...
	*/
	AreaMode(const std::vector < QVector3D > &pts)
	{
		Init(pts);
...
private:
	...
	*/
	void Init(const std::vector < QVector3D > &pts);
	/*!
	...
	*/
	QVector3D Move(QVector3D start, QVector3D end);
