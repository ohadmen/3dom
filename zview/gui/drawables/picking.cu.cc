...
// std::array<float,3> pickFromMesh(const float3* v, const int32_t* f, const std::array<float,3> &p, const std::array<float,3> &n)
// {
...
__global__ meshPickerKernel(const float3* v, size_t nv, const int3* f size_t nf, float3 p, float3 n)

MeshPicker::MeshPicker(size_t nv, size_t nf) : m_nv(nv), m_nf(nf),m_res(nf) {}
    void MeshPicker::setVptr(float3 *p) { m_v = p; }
    void MeshPicker::setFptr(float3 *p) { m_f = p; }

std::array<float, 3> MeshPicker::pick(const std::array<float,3> &p, const std::array<float,3> &n)
{
